...
3. 열리는 `config/nas-pc.local.env` 에서 수정:
   - `NAS_SSH_HOST` — `ohola.synology.me` 또는 **Tailscale IP** (`100.x.x.x`, 밖에서도 편함)
...
| 방법 | 설명 |
|------|------|
| **`scripts\NAS-접속.bat` 더블클릭** | SSH 접속 |
| **`scripts\NAS-업데이트.bat` 더블클릭** | 나스에서 pull + docker 재빌드 (SSH 안 들어가도 됨) |
| `.\scripts\nas-pc.ps1 connect` | SSH 접속 |
| `.\scripts\nas-pc.ps1 update` | 원격 업데이트 |
| `.\scripts\nas-pc.ps1 logs` | docker 로그 |
| `ssh saenggibu-nas` | setup 후 어디서든 별칭으로 접속 |
...
type $env:USERPROFILE\.ssh\id_ed25519.pub | ssh your_user@ohola.synology.me "mkdir -p ~/.ssh && cat >> ~/.ssh/authorized_keys"
```

`config/nas-pc.local.env` 에 추가:

```env
NAS_SSH_KEY=C:\Users\본인계정\.ssh\id_ed25519
```
