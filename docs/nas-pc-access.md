...
| 어디서 | NAS 접속 | 배포·접속 |
|--------|----------|-----------|
| **사무실** (NAS 설치됨) | 로컬 IP | `NAS-업데이트-로컬.bat` · `-Profile local` |
| **집 PC** | Tailscale | `NAS-업데이트.bat` · `-Profile remote` |

스크립트 이름의 **로컬** = NAS와 **같은 LAN**, **remote(기본)** = **Tailscale** 입니다.  
「집=로컬」이 아니라 **「NAS 옆=로컬」** 이라고 보면 됩니다.
...
3. 열리는 `config/nas-pc.local.env` 에서 수정:
   ...
   - `NAS_SSH_HOST` — **Tailscale IP** (`100.x.x.x`, 집 PC 등 원격)
   - `NAS_SSH_HOST_LOCAL` — **사무실 로컬 IP** (NAS와 같은 LAN, 기본 `169.254.158.191`)
...
| 방법 | 설명 |
|------|------|
| **`scripts\NAS-접속-로컬.bat`** | 사무실 — 로컬 IP SSH |
| **`scripts\NAS-업데이트-로컬.bat`** | 사무실 — pull + docker 재빌드 |
| **`scripts\NAS-배포-로컬.bat`** | 사무실 — 배포 (위와 동일) |
| **`scripts\NAS-드라이브-로컬.bat`** | 사무실 — `\\로컬IP\docker` → Y: |
| **`scripts\NAS-접속.bat`** | 집 PC — Tailscale SSH |
| **`scripts\NAS-업데이트.bat`** | 집 PC — Tailscale로 원격 업데이트 |
| `ssh saenggibu-nas-local` | 로컬(LAN) 별칭 |
...
| `docker: command not found` (SSH update) | `update` copies latest scripts via SMB first. Ensure `map -Profile local` works. |
| `$'\r': command not found` / `set: invalid option` | Windows CRLF in `.sh` on NAS. Run `git pull` then `update` again (syncs LF). Or: `sed -i 's/\r$//' /volume1/docker/saenggibu/scripts/nas-docker-update.sh` on NAS |
| SSH `Bad permissions` on `~/.ssh/config` | `.\scripts\nas-pc.ps1 fix-ssh` (removes UNKNOWN/extra ACL entries) |
